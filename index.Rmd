...
author: "Emma Lucas"
description: "Terrorist Ideology, Violence Type, and Selectivity"
output:
...
#Creation of Binary Varibles

DTV_1_2009_1 <- DTV_1_2009_1 |>
  mutate(neo = if_else(ideology == 4, "Neo Nazi", "Not Neo Nazi"))

DTV_1_2009_1 <- DTV_1_2009_1 |>
  mutate(vio = if_else(violencetype == 2, "Organised Terrorism", "Urban Political Violence or Diffuse Terrorism"))

DTV_1_2009_1 <- DTV_1_2009_1 |>
  mutate(select = if_else(selectivity == 3, "Social Group Killings", "Non-Social Group Killings"))
         

#Diff in Means for Violence Type
diff_vio <- DTV_1_2009_1|>
  specify(vio ~ neo) |>
...
#Diff in Means for Selectivity
diff_select <- DTV_1_2009_1|>
  specify(select ~ neo) |>
  hypothesise(null = "point") |>
  generate(reps = 1000, type = "permute") |>
  calculate(stat = "diff in means")
...
#As seen from the "Violence Types Across Ideology" graph, there are almost triple the number of terrorist groups with Ideology 1 (Nationalist(pro-secession)) compared to the second largest pool of terrorist groups, Ideology 0 (Vigilante (against secession)). From greatest to least with the remaining groups, Ideology 3 (Extreme-right), Ideology 2 (Extreme-left), and Ideology 4 (Neonazi) are a bit less than half that of ideology 0. 

As seen from the "Violence Types Across Ideology" graph, we see the largest concentration of attacks around Ideology 1 (Nationalist) and Violence Type 2 (organized terrorism). We also see a large concentration of attacks around Ideology 0 (Vigilante) and Violence Type 2 (organised terrorism). When looking across the horizontal axis, it can be said that Violence Type 2 (organised terrorism) is the most concentrated overall compared to Violence Type 0 (urban political violence) and 1 (diffuse terrorism), regardless of ideology. Besides Ideology 4 (neonazi) and Violence Type 1 (diffuse terrorism), there is not a dense concentration of attacks in either Violence Type 0 (urban political violence) or Violence Type 1 (diffuse terrorism). Ideology 0 (Vigilante) and Violence Type 1 (diffuse terrorism) have no points, nor does Ideology 4 (Neonazi) and Violence Type 2 (organised terrorism). 

When comparing this to the "Selectivity Across Ideology" graph, which measures attack selectivity across ideology, we observe the largest concentration in Ideology 1 (Nationalist) and Selectivity 2 (state killing). The second largest concentration is in Ideology 0 (Vigilante) and Selectivity 3 (group killing). When looking across the horizontal axis, it is less clear what Selectivity is most concentrated, although upon observation it appears to be either Selectivity 2 (state killing) or Selectivity 3 (group killing). 

## Conclusion: 1 paragraph (i) summarizing results and assessing the extent to which you find support for your hypothesis; (ii) describing limitations of the analysis and threats to inference (missing data, confounding, etc), and stating how you could improve your analysis if you had more time/money. (2pts)

If the data supported my hypothesis on violence types, I would have seen more overlap between ideologies 2 (extreme-left), 3 (extreme-right), and 4 (neonazi) and Violence Type 0 (urban political violence). Instead, we see very little victims in this section, instead falling into Violence Type 2 (organised terrorism). 

#If the data supported my hypothesis on violence types, I would have seen more overlap between ideologies 2 (extreme-left), 3 (extreme-right), and 4 (neonazi) and Violence Type 0 (urban political violence). Instead, we see very little victims in this section, instead falling into Violence Type 2 (organised terrorism).

A major limitation of my analysis is that correlation does not imply causation. Since this was not a randomised control trial (and rather mimics an observational study,) it is hard to say whether ideology is the direct cause of certain violence types and selectivity. In observational studies, researchers observe a naturally assigned treatment, and it is very commonly the case that you often cannot randomise them for ethical and logistical reasons. This raises questions of internal validity, which addresses if causal assumptions are satisfied and if we can interpret this as a causal effect. RCTs usually have higher internal validity, and observational studies have less because the treatment/control groups differ in ways that are hard to measure.
